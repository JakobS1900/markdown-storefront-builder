...
| A new IndexedDB object store holding picture bytes (as `ArrayBuffer`, not `Blob`: see data-model.md, `fake-indexeddb` does not round trip a Blob and Safari has shipped versions that returned one unreadable), in an app whose persistence layer is 111 lines and assumes a page is a few kilobytes of text. | Pictures on the device cannot be held anywhere else, and the feature is the owner's stated priority. | Keeping pictures as remote addresses only, which is the status quo, does not answer the request: a menu file whose pictures live on somebody else's server is not an offline menu. |
