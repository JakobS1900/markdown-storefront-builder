...
- [ ] T051 Drive it in a browser at 390px following `quickstart.md`, including the check that matters most: the store name reaches the **Copy tab**, not just the editor. **NOT DONE ON A DESKTOP BROWSER: the Chrome automation extension was unresponsive all session, four attempts.** Its substance was covered twice over and that is recorded rather than assumed: `npm run contrast` drives a real headless Chrome at 390 by 844 through all seven screens in both palettes and asserts no sideways scroll, and T052 drove the whole flow on the handset including the Copy tab check, which is the stronger evidence since it is the target device. What is still owed is the desktop pass specifically.
- [x] T052 Drive it on the handset following `quickstart.md` and `docs/WORKFLOW.md`. Check `mWakefulness` immediately before each `screencap`, and set `svc power stayon` back to `false` afterwards.
